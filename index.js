...
app.get('/api/cek-status-deposit', async (req, res) => {
    ...
    try {
        ...
        const statusResponse = await vpediaAPI.get(`/deposit/status?id=${transaction.vPediaDepositId}`);
        ...
        if (statusResponse.data.success && statusResponse.data.data.status === 'success') {
            ...
            const orderResponse = await vpediaAPI.get(`/order/create?code=${transaction.productCode}&tujuan=${transaction.target}`);
